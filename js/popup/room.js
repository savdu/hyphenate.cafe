...
/* -------------------------------------------------------------------------
   ...
   Everything is kept deliberately narrow (~30 columns) because the homepage
   scene at ~46 columns is already wider than an iPhone SE, and a decorative
   banner that needs sideways scrolling isn't decorative.
   ------------------------------------------------------------------------- */
...
async function openGuest(g, isMe, refresh, onLeft) {
    ...
    h('div.stack.center', {},
      ...
      isMe
        ? h('div.sheet-actions', {},
            ...
            h('button', { onclick: () => done('note') }, g.note ? 'edit your note' : 'add a note'),
        ...
        : h('div.stack', {},
            h('div.muted.small', {}, 'saying hi is coming soon — for now, go say it in person'),
  ...
  if (chosen === 'activity') {
    ...
    const note = await editNote(g.note);
    if (note !== null) await guest.setNote(note);
...
export function mountRoom(root, { onLeft }) {
  ...
  const draw = () => {
    ...
    const said = guests.filter(g => g.note).sort((a, b) => b.noteAt - a.noteAt);
    render(log, said.length
      ? said.map(g => h('div.log-line', {},
          h('span.who', {}, g.name),
          ' ',
          h('span.said', {}, g.note),
          h('span.muted.small', {}, ` ${timeOf(g.noteAt)}`)
        ))
  ...
  /* Coming back to a phone that was asleep.
     ...
     Only after a real absence: this costs a read per guest, and charging for
     that every time someone glances at another app and back would add up
     across an afternoon. A quick flick away changes nothing worth paying for. */
  const AWAY_ENOUGH = 5000;
  let hiddenAt = 0;

  const resync = async () => {
    try { apply(await store.getGuests()); }
    catch (err) { console.warn('[room] could not re-read the room on wake', err); }
  };
